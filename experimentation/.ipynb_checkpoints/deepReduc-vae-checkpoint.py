...
t = pd.read_csv("pendigits.tra", header=None)
x_train = t.drop(16, axis=1)
y_train = t[16]
print(x_train.shape)
...
def deepduc(sub_train, l, nb):

	def custom_loss(y_true, y_pred):
	    bottle=y_pred[:,:4]
	    pred = y_pred[:,4:20]
	    Sb = y_pred[:,20:]
	    return K.sqrt(K.sum(K.square(pred - y_true), axis=-1)) + l*K.sqrt(K.sum(K.square(bottle - Sb), axis=-1))

	input_bottleneck = Input(shape=(4,), name="inp_bt")
	input_img = Input(shape=(16,), name="inp_img")
	input_S = Input(shape=(4,), name="inp_S")


	bottlenek = Dense(4, activation='relu', name="bottleneck")(input_img)
	...
	output = Dense(16, activation='sigmoid', name="logits")(bottlenek)
	...
	model.compile(optimizer='adadelta', loss={"concatenate":custom_loss, "logits": None})

	for epoch in range(0,10):
	    for x in sub_train[:-1]:
	  
	        bottleneck = bottleneck_model.predict(x)
	        
	        lle = LLEC(n_components=4, n_neighbors=nb)
	        S = lle.fit_W(bottleneck)
	        sdotB = np.dot(S, bottleneck)
	        
	        lss = model.train_on_batch([bottleneck, x,sdotB ], x)


	bt_train = bottleneck_model.predict(x_train, verbose=1)
	p = "result_pendigits/l"+str(l)+"_n"+str(nb)
	os.makedirs(p, exist_ok=True)
	pd.DataFrame(bt_train).to_csv(p+"/bottlneck.csv", index=False, header=None)
	# pd.DataFrame(bt_train).to_csv("current_bottleneck.csv", index=False)
...
# deepduc(sbtr,0.2,3,3)
# deepduc(sbtr,0.8,3,3)
# deepduc(sbtr,2,3,3)
# deepduc(sbtr,2,5,3)

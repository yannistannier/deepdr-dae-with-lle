...
def deepduc(sub_train, l, nb):
	...
	def custom_loss(y_true, y_pred):
		...
		# return K.sqrt(K.sum(K.square(y_true - pred), axis=-1)) 
		# return K.sqrt(K.sum(K.square(y_true - pred), axis=-1)) + 0*K.sqrt(K.sum(K.square(bottle - Sb), axis=-1))
	...
	# output = Dense(794, activation='sigmoid', name="logits")(bottlenek)
	...
	model = Model([input_bottleneck, input_img, input_S], final_dense)

	bottleneck_model = Model(inputs=model.get_layer("inp_img").input, outputs=model.get_layer("bottleneck").output)

	model.compile(optimizer='adadelta', loss={"concatenate":custom_loss})
	...
	p = "resultat/crossentropy/l"+str(l)+"_n"+str(nb)
